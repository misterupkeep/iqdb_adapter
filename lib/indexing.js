...
export async function index(file, url) {
  ...
  await axios.post(`${process.env.IQDB_HOST || ""}/images/${id}`, formdata, {
    headers: {
...
export async function reindex_uploads() {
  const url_regex = /.*(original\/.*)/;
  const ext_regex = /\.(jpe?g|png|webp)$/;

  const uploads_folder =
    process.env.UPLOADS_ORIGINAL_PATH || "./uploads/original/";

  console.log(`[info] starting reindexing uploads folder ${uploads_folder}`);

  await walk(uploads_folder, async (err, path, dirent) => {
    // For all image files
    if (dirent.isFile() && ext_regex.test(dirent.name)) {
      const url = (process.env.LYCHEE_PREFIX || "") + url_regex.exec(path)[1];
      const id = hash24(url);
  ...
  console.log(`[info] finished indexing uploads folder ${uploads_folder}`);
}
